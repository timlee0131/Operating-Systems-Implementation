...
void release(int playerID, int resource, int position = A + K) {
...
bool request(int playerID, int resource, bool preEmption = false) {
    pthread_mutex_lock(&printLock);
    ...
    // print graph
    pthread_mutex_lock(&printLock);
    avoidanceGraph->printGraph(playerID, resource);
    pthread_mutex_unlock(&printLock);

    if(preEmption) {
        for(int i = 1; i <= K; i++) {
            int processPM = avoidanceGraph->find(i) - K;

            if(processPM >= 0) {
                release(processPM, i);
                avoidanceGraph->removeEdge(playerID + 1 + K, resource);
                avoidanceGraph->addEdge(i, playerID + 1 + K);
                if(!avoidanceGraph->isCyclic()) {
                    avoidanceGraph->addEdge(processPM + 1 + K, i);

                    pthread_mutex_lock(&printLock);
                    std::cout << playersResources[playerID].playerName << " preempted " << i << " from " << playersResources[processPM].playerName << std::endl;
                    pthread_mutex_unlock(&printLock);
                    ...
                    avoidanceGraph->removeEdge(i, playerID + 1 + K);
                    avoidanceGraph->addEdge(i, processPM + 1 + K);
    ...
    while(!endOfGame) {
        ...
        for(int i = 0; i < playersResources[currentId].resources.size(); i++) {
            ...
            if(resource > 0) {
                ...
                if(preEmptionCounter >= 3) {
                    pthread_mutex_lock(&graphLock);
                    requestReturn = request(currentId, resource, true);
                    pthread_mutex_unlock(&graphLock);
                } else {
                    pthread_mutex_lock(&graphLock);
                    requestReturn = request(currentId, resource);
                    pthread_mutex_unlock(&graphLock);
                }
                ...
                if(requestReturn) {
                    ...
                    preEmptionCounter++;
                    pthread_mutex_lock(&printLock);
                    std::cout << playersResources[currentId].playerName << "requests resource " << resource << " : denied " << preEmptionCounter << " times" << std::endl;
                    pthread_mutex_unlock(&printLock);
                    // sleepy(1 + q/100);
                }
            } else {
